...
def chat_with_openai(prompt):
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4-1106-preview",  # Fast GPT-4 variant
            messages=[
...
# === GUI Setup ===
window = tk.Tk()
window.title("Voice Assistant | AssemblyAI + GPT-4.1 Nano + Azure TTS")
window.geometry("950x620")
window.config(bg="#f2f2f2")

header = tk.Frame(window, height=60, bg="#007acc")
header.pack(fill="x")
tk.Label(header, text="🎤 Assistant | AssemblyAI STT + GPT-4.1 Nano + Azure TTS",
